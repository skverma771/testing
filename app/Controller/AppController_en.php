...
{
    public $components = array("Security", "Auth", "Acl.AclFilter", "XAjax", "RequestHandler", "Cookie", "Cms");
    public $helpers = array("Html", "Form", "Javascript", "Session", "Text", "Js", "Time", "Layout", "Auth");
    public $uses = array("Block", "Link", "Setting", "Node");
    ...
    public function __construct($request = null, $response = null)
    {
        ...
        if (!defined("STDIN") && $_SERVER["HTTP_HOST"] != "localhost" && strstr($_SERVER["HTTP_HOST"], ".dev1.agriya.com") != ".dev1.agriya.com" && strstr($_SERVER["HTTP_HOST"], ".dev.agriya.com") != ".dev.agriya.com" && strstr($_SERVER["HTTP_HOST"], ".cssilize.com") != ".cssilize.com" && !defined("LICENSE_HASH")) {
    ...
    public function beforeFilter()
    {
        ...
        if ($this->Auth->user("id")) {
            $this->loadModel("User");
            $user = $this->User->find("first", array("conditions" => array("User.id" => $this->Auth->user("id")), "recursive" => 0 - 1));
        }
        ...
        if (Configure::read("User.signup_fee") && $this->Auth->user("id") && $this->Auth->user("role_id") != ConstUserTypes::Admin) {
            $fee_urls = array("users/login", "users/logout", "payments/user_pay_now", "payments/get_gateways", "sudopays/process_ipn", "sudopays/success_payment", "sudopays/cancel_payment", "messages/activities", "users/show_header", "nodes/view");
            if (!in_array($cur_page, $fee_urls) && empty($user["User"]["is_paid"])) {
                $this->redirect(array("controller" => "payments", "action" => "user_pay_now", $this->Auth->user("id"), $this->User->getActivateHash($this->Auth->user("id"))));
            }
        ...
        if (isPluginEnabled("SocialMarketing") && empty($this->request->params["requested"]) && !empty($is_redirect_to_social_marketing)) {
            $skip_urls = array("users/login", "users/logout", "devs/asset_js", "devs/asset_css", "social_marketings/import_friends", "social_contacts/index", "social_contacts/update", "users/follow_friends", "user_followers/add_multiple", "messages/activities", "users/show_header", "social_marketings/publish_success");
            if ($this->Auth->user("id") && !in_array($cur_page, $skip_urls) && !empty($user) && (!$user["User"]["is_skipped_fb"] || !$user["User"]["is_skipped_twitter"] || !$user["User"]["is_skipped_google"] || !$user["User"]["is_skipped_yahoo"])) {
                if (!$user["User"]["is_skipped_fb"]) {
                    ...
                    if (!$user["User"]["is_skipped_twitter"]) {
                        $type = "twitter";
                    } else {
                        if (!$user["User"]["is_skipped_google"]) {
                            $type = "gmail";
                        } else {
                            if (!$user["User"]["is_skipped_yahoo"]) {
                                $type = "yahoo";
                            } else {
                                if (!$user["User"]["is_skipped_linkedin"]) {
                                    $type = "linkedin";
                                }
                            }
                        }
                ...
                $this->redirect(array("controller" => "social_marketings", "action" => "import_friends", "type" => $type, "admin" => false));
            }
        }
        if (isPluginEnabled("LaunchModes")) {
            $pre_launch_exception_array = array("subscriptions/add", "subscriptions/check_invitation", "subscriptions/confirmation", "users/logout", "users/facepile", "nodes/view", "projects/index", "pages/view", "images/view", "devs/asset_js", "devs/asset_css", "devs/robots", "devs/sitemap", "users/show_captcha", "users/captcha_play", "payments/user_pay_now", "payments/get_gateways", "users/show_header");
            if ($this->Auth->user("role_id") != ConstUserTypes::Admin && Configure::read("site.launch_mode") == "Pre-launch" && !in_array($cur_page, $pre_launch_exception_array) && empty($this->request->params["prefix"])) {
                $this->redirect("/");
            }
            $private_beta_exception_array = array_merge($pre_launch_exception_array, array("users/login", "users/logout", "users/register", "users/admin_login", "users/show_header", "users/forgot_password", "users/activation", "users/reset", "payments/user_pay_now", "payments/_processPayment", "payments/success_payment", "payments/cancel_payment", "payments/get_gateways"));
            if ($this->Auth->user("role_id") != ConstUserTypes::Admin && Configure::read("site.launch_mode") == "Private Beta" && !in_array($cur_page, $private_beta_exception_array) && !$this->Auth->user("id")) {
        ...
        $cur_page = $this->request->params["controller"] . "/" . $this->request->params["action"];
        $maintenance_exception_array = array("devs/asset_js", "devs/asset_css", "devs/robots", "devs/sitemap", "users/show_header");
        ...
        if (Configure::read("site.is_ssl_enabled")) {
            $secure_array = array("users/login", "users/admin_login", "users/register", "users/add_to_wallet", "payments/project_pay_now", "payments/user_pay_now", "payments/order", "projects/project_pay_now");
            $cur_page = $this->request->params["controller"] . "/" . $this->request->params["action"];
            if (in_array($cur_page, $secure_array)) {
                $this->Security->blackHoleCallback = "forceSSL";
                $this->Security->requireSecure($this->request->params["action"]);
            } else {
                if (env("HTTPS") && !$this->RequestHandler->isAjax()) {
                    $this->_unforceSSL();
                }
            }
        }
        ...
        if (isset($_SERVER["HTTP_USER_AGENT"]) && (strtolower($_SERVER["HTTP_USER_AGENT"]) == "shockwave flash" || strpos(strtolower($_SERVER["HTTP_USER_AGENT"]), "adobe flash player") !== false) && isset($this->request->params["pass"][0]) && $this->request->action == "flashupload") {
        ...
        if ($this->Auth->user("id")) {
            ...
            $user = $user_model_obj->find("first", array("conditions" => array("User.id =" => $this->Auth->user("id")), "contain" => array("UserAvatar", "UserProfile" => array("City" => array("fields" => array("City.name")), "Country" => array("fields" => array("Country.name", "Country.iso_alpha2")))), "recursive" => 2));
            $this->set("logged_in_user", $user);
            $count_conditions = array();
            $count_conditions = array("Project.user_id" => $user["User"]["id"], "Project.is_admin_suspended !=" => 1, "Project.is_active" => 1);
            $conditions = array();
            $response = Cms::dispatchevent("Controller.ProjectType.getConditions", $this, array("type" => "project_count", "page" => "userview"));
            $countContain = array();
            ...
            $project_count = $user_model_obj->Project->find("count", array("conditions" => $conditions, "contain" => $countContain, "recursive" => 0));
            $response = Cms::dispatchevent("Controller.ProjectType.getConditions", $this, array("type" => "all_project_count", "page" => "userview"));
            if (!empty($response->data["conditions"])) {
                $conditions = array_merge($count_conditions, $response->data["conditions"]);
            }
            $all_project_count = $user_model_obj->Project->find("count", array("conditions" => $conditions, "contain" => $countContain, "recursive" => 0));
            $this->set("project_count", $project_count);
            $this->set("all_project_count", $all_project_count);
            $conditions = array();
            $response = Cms::dispatchevent("Controller.ProjectType.getConditions", $this, array("type" => "idea_count", "contain" => $countContain, "page" => "userview"));
            if (!empty($response->data["conditions"])) {
                $conditions = array_merge($count_conditions, $response->data["conditions"]);
            }
            $idea_count = $user_model_obj->Project->find("count", array("conditions" => $conditions, "recursive" => 0));
            $this->set("idea_count", $idea_count);
            $count_conditions = array();
            $count_conditions = array("Project.is_admin_suspended !=" => 1, "Project.is_active" => 1);
            if (isPluginEnabled("ProjectFollowers")) {
                $projectIds = $user_model_obj->Project->ProjectFollower->find("list", array("conditions" => array("ProjectFollower.user_id" => $user["User"]["id"]), "fields" => array("ProjectFollower.project_id", "ProjectFollower.project_id"), "recursive" => 0 - 1));
                $count_conditions["Project.id"] = $projectIds;
            }
            $response = Cms::dispatchevent("Controller.ProjectType.getConditions", $this, array("type" => "count", "page" => "userview"));
            if (!empty($response->data["conditions"])) {
                $conditions = array_merge($count_conditions, $response->data["conditions"]);
            }
            $project_following_count = $user_model_obj->Project->find("count", array("conditions" => $conditions, "contain" => $countContain, "recursive" => 0));
        ...
        $this->is_wallet_enabled = $payment_gateway->find("count", array("conditions" => array("PaymentGateway.id =" => ConstPaymentGateways::Wallet, "PaymentGateway.is_active =" => 1), "recursive" => 0 - 1));
        $this->set("is_wallet_enabled", $this->is_wallet_enabled);
    }
    public function _unum()
    {
    ...
    public function _redirectGET2Named($whitelist_param_names = null)
    {
        ...
        if (is_array($whitelist_param_names)) {
            foreach ($whitelist_param_names as $param_name) {
                if (isset($this->request->query[$param_name])) {
    ...
    public function _redirectPOST2Named($whitelist_param_names = null)
    {
        $query_strings = array();
        $model = Inflector::classify($this->request->params["controller"]);
        if (is_array($whitelist_param_names)) {
            foreach ($whitelist_param_names as $param_name) {
                if (isset($this->request->data[$model][$param_name])) {
            ...
            if ($actionid && !empty($ids)) {
                switch ($actionid) {
                    case ConstMoreAction::Inactive:
                        ...
                        $this->{$this->modelClass}->updateAll(array($this->modelClass . "." . $field_name => 0), array($this->modelClass . ".id" => $ids));
                    ...
                    case ConstMoreAction::Active:
                        ...
                        $this->{$this->modelClass}->updateAll(array($this->modelClass . "." . $field_name => 1), array($this->modelClass . ".id" => $ids));
                        if ($this->modelClass == "User") {
                            foreach ($ids as $key => $user_id) {
                    ...
                    case ConstMoreAction::Delete:
                        ...
                        if ($this->modelClass == "Message") {
                            ...
                            $this->{$this->modelClass}->deleteAll(array($this->modelClass . ".id" => $ids));
                        }
                        if ($this->modelClass == "User") {
                            foreach ($ids as $key => $user_id) {
                    ...
                    case ConstMoreAction::Suspend:
                        if ($this->modelClass == "Message") {
                            $this->{$this->modelClass}->MessageContent->updateAll(array("MessageContent.is_admin_suspended" => 1), array("MessageContent.id" => $ids));
                        } else {
                            $this->{$this->modelClass}->updateAll(array($this->modelClass . ".is_admin_suspended" => 1), array($this->modelClass . ".id" => $ids));
                        }
                        $this->Session->setFlash(__l("Checked records has been suspended"), "default", null, "success");
                        break;
                    case ConstMoreAction::Unsuspend:
                        if ($this->modelClass == "Message") {
                            $this->{$this->modelClass}->MessageContent->updateAll(array("MessageContent.is_admin_suspended" => 0), array("MessageContent.id" => $ids));
                        } else {
                            $this->{$this->modelClass}->updateAll(array($this->modelClass . ".is_admin_suspended" => 0), array($this->modelClass . ".id" => $ids));
                        }
                        $this->Session->setFlash(__l("Checked records has been unsuspended"), "default", null, "success");
                        break;
                    case ConstMoreAction::Flagged:
                        if ($this->modelClass == "Message") {
                            $this->{$this->modelClass}->MessageContent->updateAll(array("MessageContent.is_system_flagged" => 1), array("MessageContent.id" => $ids));
                        } else {
                            $this->{$this->modelClass}->updateAll(array($this->modelClass . ".is_system_flagged" => 1), array($this->modelClass . ".id" => $ids));
                        }
                        $this->Session->setFlash(__l("Checked records has been flagged"), "default", null, "success");
                        break;
                    case ConstMoreAction::Unflagged:
                        if ($this->modelClass == "Message") {
                            $this->{$this->modelClass}->MessageContent->updateAll(array("MessageContent.is_system_flagged" => 0), array("MessageContent.id" => $ids));
                        } else {
                            $this->{$this->modelClass}->updateAll(array($this->modelClass . ".is_system_flagged" => 0), array($this->modelClass . ".id" => $ids));
                        }
                    ...
                    case ConstMoreAction::Export:
                        ...
                        $this->redirect(array("controller" => "users", "action" => "export", "ext" => "csv", $hash, "admin" => true), true);
                }
            }
        }
    ...
    public function admin_update_status($id = null, $action = null)
    {
        ...
        if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "active") {
            ...
            if (isset($this->{$this->modelClass}->_schema["is_approved"])) {
                ...
                if ($this->request->params["controller"] == "blocks" || $this->request->params["controller"] == "nodes" || $this->request->params["controller"] == "links") {
                    $field_name = "status";
                } else {
                    if ($this->request->params["controller"] == "Project") {
                        $field_name = "is_system_flagged";
                    } else {
                        if ($this->request->params["controller"] == "blogs") {
                            $field_name = "is_published";
                        }
                    }
                }
            ...
            if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "deactivate") {
                ...
                if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "userflag-deactivate") {
                    ...
                    if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "approved") {
                        $field_name = "is_approved";
                        $_data = array();
                        $_data[$this->modelClass]["id"] = $id;
                        $_data[$this->modelClass][$field_name] = 1;
                        ...
                        if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "inactive") {
                            $field_name = "is_active";
                            if (isset($this->{$this->modelClass}->_schema["is_approved"])) {
                                $field_name = "is_approved";
                            } else {
                            ...
                            if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "disapproved") {
                                $field_name = "is_approved";
                                $_data = array();
                                $_data[$this->modelClass]["id"] = $id;
                                $_data[$this->modelClass][$field_name] = 0;
                                ...
                                if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "flag") {
                                    ...
                                    if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "unflag") {
                                        if ($this->modelClass == "Message") {
                                            $data = array();
                                        ...
                                        if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "suspend") {
                                            if ($this->modelClass == "Message") {
                                                $data = array();
                                            ...
                                            if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "unsuspend") {
                                                if ($this->modelClass == "Message") {
                                                    $data = array();
                                                ...
                                                if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "featured") {
                                                    ...
                                                    if (!empty($this->request->params["named"]["status"]) && $this->request->params["named"]["status"] == "notfeatured") {
                                                        $field_name = "is_featured";
                                                        $_data = array();
                                                        $_data[$this->modelClass]["id"] = $id;
                                                        $_data[$this->modelClass][$field_name] = 0;
        ...
        if (!empty($this->request->query["r"])) {
            ...
            if ($this->request->params["controller"] == "links" && $this->request->params["action"] == "admin_update_status") {
                $this->redirect(array("controller" => $this->request->controller, "action" => "index", $this->request->params["named"]["menu_id"]));
            } else {
                if ($this->request->params["controller"] == "projects" && !empty($this->request->params["named"]["project_type"])) {
                    $this->redirect(array("controller" => Inflector::pluralize($this->request->params["named"]["project_type"]), "action" => "index"));
                } else {
    ...
    public function captcha_play($session_var = null)
    {
        ...
        $this->disableCache();
        $this->RequestHandler->respondAs("mp3", array("attachment" => "captcha.mp3"));
    ...
    public function _affiliate_referral()
    {
        if (!empty($this->request->params["named"]["r"])) {
            $this->loadModel("User");
            $referrer = array();
            $user = $this->User->find("first", array("conditions" => array("User.username" => $this->request->params["named"]["r"], "User.is_affiliate_user" => 1), "fields" => array("User.username", "User.id"), "recursive" => 0 - 1));
            if (!empty($user)) {
                ...
                if (!empty($this->request->params["controller"]) && $this->request->params["controller"] == "jobs") {
                    if (!empty($this->request->params["named"]["category"])) {
                        ...
                        if (!empty($this->request->params["action"]) && $this->request->params["action"] == "view") {
                            $referrer["refer_id"] = $user["User"]["id"];
                            $referrer["type"] = "view";
                ...
                $this->redirect(array("controller" => $this->request->params["controller"], "action" => $this->request->params["action"], "r_value" => $r_value, $params));
            }
        }
    }
}
