...
<div class="projectFunds form clearfix row-fluid">
  <div class="top-space">
...
<div class="clearfix">
  ...
  <?php
		...
		echo $this->Form->input('user_name',array('label' => __l('Name'),'required'=>'required', 'oninvalid'=>"this.setCustomValidity('Enter your name')"));;
		}
		?>
  ...
  <div class="group-block hor-space">
  <?php
          if (!empty($pledge['Pledge']['pledge_type_id'])&&($pledge['Pledge']['pledge_type_id'] != ConstPledgeTypes::Reward)) {
        ...
        ?>
  <div class="rewards top-space">
    <?php if ($disabled) :?>
    <div class="input radio">
    <div class="round-3 sold-out <?php echo $disabled;?>"><input type="radio" disabled="true" /><span class="label label-inverse hor-mspace"><?php echo __l('Sold out');?></span>
    <span><?php echo $this->Html->siteCurrencyFormat($this->Html->cCurrency($reward['pledge_amount'], false)).' + '; ?></span> <span class="help"><?php echo $this->Html->cText($reward['reward'], false); ?><span class="blackc"><?php if(isset($reward['is_shipping']) && $reward['is_shipping']) { echo ' ('.__l('Estimated delivery date') . ': ' . $this->Html->cDate($reward['estimated_delivery_date']).')'; } ?></span></span></div></div>
    <?php else:?>
    <?php
    $strDeliveryDate = '';
  ...
  <div class="profile-block clearfix">
    ...
    <div class="pull-right js-side-map-div <?php echo $class;?>">
  ...
  <div class="span12">
    ...
    <div class="group-block hor-space personal-radio"> <?php echo $this->Form->input('is_anonymous',array('type' =>'radio','options'=>$radio_options,'default'=>ConstAnonymous::None,'legend'=>false));?> </div>
    </fieldset>
  </div>  
  ...
  <?php echo $this->element('pledge-faq', array('cache' => array('config' => 'sec')),array('plugin' => 'Pledge')); ?>
    ...
    <?php
    echo $this->element('payment-get_gateways', array('model'=>'ProjectFund','type'=>'is_enable_for_pledge','is_enable_wallet'=>0,'project_type'=>$project['ProjectType']['name'],'user_id'=>$project['Project']['user_id'], 'cache' => array('config' => 'sec')));?>
