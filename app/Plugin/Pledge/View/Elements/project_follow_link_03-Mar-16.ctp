		...
		if (isPluginEnabled('ProjectFollowers')) {
			if(isPluginEnabled('HighPerformance') && (Configure::read('HtmlCache.is_htmlcache_enabled') || Configure::read('cloudflare.is_cloudflare_enabled'))) {?>
							<div class="alpuf-<?php echo $project['Project']['id'];?> pull-left hide">
								<?php echo $this->Html->link("<i class='icon-ok'></i> ". __l('Following'), array('controller' => 'project_followers', 'action' => 'delete', $follower['ProjectFollower']['id']),array('class'=>"show btn span3 js-add-remove-followers js-no-pjax js-tooltip js-unfollow",'escape' => false, 'title'=>__l('Unfollow')));  ?>
							</div>
							...
							<div class='alpf-<?php echo $project['Project']['id'];?> pull-left hide'>
								<a class="btn span2 js-tooltip" title="<?php echo __l('Follow');?>" href="<?php echo $redirect_url; ?>"><?php echo __l('Follow');?></a>
							</div>
							<div class='blpuf-<?php echo $project['Project']['id'];?> pull-left hide'>
									<?php echo $this->Html->link(__l('Follow'), array('controller' => 'users', 'action' => 'login', '?' => 'f=project/' . $project['Project']['slug'], 'admin' => false), array('class' => 'btn span2 tooltiper js-tooltip', 'title' => __l('Follow')));	?>
							</div>
			<?php } else {

			if ($this->Auth->sessionValid()):
				if (!empty($follower)):
					echo $this->Html->link("<i class='icon-ok'></i> ". __l('Following'), array('controller' => 'project_followers', 'action' => 'delete', $follower['ProjectFollower']['id']),array('class'=>"show btn span3 js-add-remove-followers js-no-pjax js-tooltip js-unfollow",'escape' => false, 'title'=>__l('Unfollow')));
				else:
			  ...
			  <?php
					...
					?>
					<span class="btn span2 js-tooltip" title="<?php echo __l('Follow');?>"><?php echo __l('Follow');?></span>
			  <?php	endif;
				endif;
			else:
				if (in_array($project['Pledge']['pledge_project_status_id'], array(ConstPledgeProjectStatus::OpenForIdea, ConstPledgeProjectStatus::OpenForFunding, ConstPledgeProjectStatus::GoalReached , ConstPledgeProjectStatus::FundingClosed))):
					echo $this->Html->link(__l('Follow'), array('controller' => 'users', 'action' => 'login', '?' => 'f=project/' . $project['Project']['slug'], 'admin' => false), array('class' => 'btn span2 tooltiper js-tooltip', 'title' => __l('Follow')));
				endif;
			endif;
		} }
	if (empty($response->data['is_allow_to_vote']) && empty($response->data['is_show_vote'])) {
