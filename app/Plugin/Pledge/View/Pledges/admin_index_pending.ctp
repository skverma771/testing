...
?>
    ...
    <?php echo $this->Html->link('<span class="show blackc"><span class="text-16 textb">'.$this->Html->cInt($step_count[$key] ?? 0, false).'</span></span><span class="label pro-status-'.$key.'">' .__l($value). '</span>', array('controller'=>'pledges','action'=>'index','project_status_id' => ConstPledgeProjectStatus::PendingAction, 'step'=> $key), array('class' => 'pull-left no-under', 'escape' => false));?>
    </li>
<?php
	}
?>
...
<section class="space clearfix">
  ...
  <table class="table table-bordered table-condensed table-hover no-mar">
    ...
    <?php
	  ...
	  <td>
	  		<div class="btn-group">
				<?php /* ?><a href="#" title="Actions" class="btn js-no-pjax"><?php echo __l('Approve?'); ?></a><?php */ ?>
				<?php
