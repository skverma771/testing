...
class Pledge extends AppModel
{
    ...
    function updateStatus($to_project_status_id, $project_id) 
    {
        ...
        $tmp_project = $this->{'processStatus' . $to_project_status_id}($project);
            ...
            if ($to_project_status_id == 2) {
                // $this->addDaysToEndDate($project);
            }
        }
		...
		function processStatus9($project) 
        {
            // Project goes back to pending action instead of expire 
			$data['Project']['project_end_date'] = date("Y-m-d", strtotime("+ 10 days"));
		...
		public function getCategoryConditions($category = null) 
		{
        ...
        public function getViewConditions() {
            // for admin account we don't need any other restrictions.
		...
		public function onProjectCategories() 
		{
        ...
        function addDaysToEndDate($project)
        { 
			$data['Project']['project_end_date'] = date("Y-m-d", strtotime("+ 7 days"));
            $data['Project']['id'] = $project['Project']['id'];
            $this->Project->save($data);
        }
}
