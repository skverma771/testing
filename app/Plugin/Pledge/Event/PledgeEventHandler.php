<?php
class PledgeEventHandler extends UtObject implements CakeEventListener
{
    /**
     * implementedEvents
     *
        ...
        if ($project['Project']['project_type_id'] == ConstProjectTypes::Pledge) {
            ...
            if (!empty($data['ProjectFund']['project_reward'])) {
                ...
                if (!empty($reward) && $data['ProjectFund']['amount'] < $reward['ProjectReward']['pledge_amount']) {
                    $this->PledgeFund->validationErrors['amount'] = sprintf(__l('You cannot select this %s for the amount you entered.') , Configure::read('project.alt_name_for_reward_singular_small'));
                }
