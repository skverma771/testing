      ...
      <div class="clearfix affiliatecashwithdrawal-block">
        <div class="grid_left">
          ...
          <div class = "input text required <?php echo $class;?>">
            <?php echo $this->Form->input('amount',array($currecncy_place => '<span class="currency add-on">'.Configure::read('site.currency').'</span>','div' => false));?>
          </div>
          <span class="info grayc"><i class="grayc icon-info-sign"></i> <?php echo sprintf(__l('Minimum withdraw amount: %s <br/> Maximum withdraw amount: %s'),$min, $max); ?></span>
          <?php
