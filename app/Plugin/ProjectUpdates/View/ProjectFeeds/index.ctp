...
<ol class="list comments-list unstyled" start="<?php echo $this->Paginator->counter(array(
  'format' => '%start%'
));?>">
<?php
...
?>
  <li<?php echo $class;?>>
  <div class="rss-title"><span class="htruncate"><?php echo $this->Html->link($projectFeed['ProjectFeed']['title'], $projectFeed['ProjectFeed']['link'], array('target' => '_blank', 'class'=>'js-tooltip js-no-pjax','title' => $projectFeed['ProjectFeed']['title'])); ?></span></div>
    <div class="clear"></div>
    <div class="rss-date">
...
?>
  <li>
  	<div class="thumbnail space dc grayc">
		<p class="ver-mspace top-space text-16"><?php echo sprintf(__l('No %s available'), sprintf(__l('%s Feeds'), Configure::read('project.alt_name_for_project_singular_caps')));?></p>
	</div>
  </li>
<?php
endif;
