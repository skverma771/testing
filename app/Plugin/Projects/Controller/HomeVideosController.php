...
class HomeVideosController extends AppController
{
	...
	public function index() 
    {
		...
		$ip_address = $this->Ip->find('first',array(
									'conditions' => array(
											'Ip.id' => $this->User->toSaveIp(),
									),
									'recursive' => -1,
							));
		$this->TotalView->create();
		$this->request->data['TotalView']['date'] = date('y:m:d h:i:s');
		$this->request->data['TotalView']['view'] = '1';
		$this->request->data['TotalView']['ip_address'] = $ip_address['Ip']['ip'];
		...
		//$this->element('footer', array('viewers' => $viewers));
		//echo $this->element('footer', array('config' => 'sec','viewers' => $viewers)); 
