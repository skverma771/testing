    ...
    public function add($project_id = null)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            //if ($this->request->data['ProjectFund']['payment_gateway_id'] != ConstPaymentGateways::Wallet && strpos($this->request->data['ProjectFund']['payment_gateway_id'], 'sp_') >= 0) {
                if ($this->request->data['ProjectFund']['payment_gateway_id'] != ConstPaymentGateways::Wallet) {
                //$this->request->data['ProjectFund']['sudopay_gateway_id'] = str_replace('sp_', '', $this->request->data['ProjectFund']['payment_gateway_id']);
                $this->request->data['ProjectFund']['payment_gateway_id'] = ConstPaymentGateways::Paypal;
            }
        }
        $contain = array(
            'Attachment',
        ...
        //     ));
        // }
        $currentProjectFund = $this->ProjectFund->find('first', array(
            'conditions' => array(
                'project_id' => $project_id
            )
        ));
        if($currentProjectFund['ProjectFund']['project_fund_status_id'] == '3') {
            $this->Session->setFlash(sprintf(__l('%s has been closed') , Configure::read('project.alt_name_for_project_singular_caps')) , 'default', null, 'error');
            $this->redirect(array(
                'controller' => 'projects',
                'action' => 'view',
        ...
        if (!empty($this->request->data)) {
			...
			if (empty($this->Auth->user('id'))) {
				//Guest user details stored the table : guest_user_funds
				$this->loadModel('GuestUser');			
//                $this->request->data['GuestUser'] = new GuestUser();

//				$this->log('Guest user details');
//				$this->log($this->request->data['ProjectFund']);
//				$this->log('Sudopay user details');
//				$this->log($this->request->data['Sudopay']);

//				$this->log($this->request->data);

                if (isPluginEnabled('Sudopay')) {
                    $this->request->data['GuestUser']['email']    = $this->request->data['Sudopay']['buyer_email'];
                    $this->request->data['GuestUser']['address']  = $this->request->data['Sudopay']['buyer_address'];
                    $this->request->data['GuestUser']['city']     = $this->request->data['Sudopay']['buyer_city'];
                    $this->request->data['GuestUser']['state']    = $this->request->data['Sudopay']['buyer_state'];
                    $this->request->data['GuestUser']['country']  = $this->request->data['Sudopay']['buyer_country'];
                    $this->request->data['GuestUser']['zip_code'] = $this->request->data['Sudopay']['buyer_zip_code'];
                    $this->request->data['GuestUser']['phone_no'] = $this->request->data['Sudopay']['buyer_phone'];
                }
                else {
                    $this->request->data['GuestUser']['email']    = Configure::read('EmailTemplate.from_email');
    /*                    $this->request->data['GuestUser']['address']  = $this->request->data['Sudopay']['buyer_address'];
		    *                                        $this->request->data['GuestUser']['city']     = $this->request->data['Sudopay']['buyer_city'];
		    *                                                            $this->request->data['GuestUser']['state']    = $this->request->data['Sudopay']['buyer_state'];
		    *                                                                                $this->request->data['GuestUser']['country']  = $this->request->data['Sudopay']['buyer_country'];
		    ...
		    *                                                                                                                        $this->request->data['GuestUser']['phone_no'] = $this->request->data['Sudopay']['buyer_phone'];*/
				...
				$this->log($this->request->data);
		//$this->log('GuestUserFund Details');
				//$this->log($this->request->data['GuestUser']);
				$this->GuestUser->save($this->request->data['GuestUser']);
				$this->request->data['ProjectFund']['guest_user_id'] = $this->GuestUser->getLastInsertID();
	  ...
	  //  $this->log($response1->data['error']);
            ...
            if ($this->ProjectFund->validates() && empty($response1->data['error'])) {
                ...
                if ($this->request->data['ProjectFund']['payment_gateway_id'] == ConstPaymentGateways::Wallet and isPluginEnabled('Wallet')) {
                ...
                } else if ($this->request->data['ProjectFund']['payment_gateway_id'] == ConstPaymentGateways::Paypal) {
                    App::import('Model', 'PaymentGateway');
					$this->PaymentGateway = new PaymentGateway();
                    $this->loadModel('User');	
                    $paypal_settings = $this->PaymentGateway->getPaymentSettings(ConstPaymentGateways::Paypal);
                    $this->log($paypal_settings,'paypal_settings');
                    ...
                    App::import('Vendor','paypal-adaptive/PPAutoloader');
					$this->PPAutoloader = new PPAutoloader();
                    $this->PPAutoloader->register($paypal_settings['PaymentGateway']['is_test_mode']);
                    $this->log($paypal_settings,'order');
                    $amount = 0;
					$admin_amount = 0;
                    $receiver_amount = 0;
                    $amount = round($projectFund['ProjectFund']['amount']);
                    $admin_amount = round($projectFund['ProjectFund']['site_fee']);
		    $receiver_amount = $amount - $admin_amount;
		    $receiver = array();
					$receiver[0] = new Receiver();
					$receiver[0]->email = $paypal_settings['PaymentGateway']['payee_account'];
					$receiver[0]->amount = round($amount);
					$receiver[0]->primary = true;
					$receiver[1] = new Receiver();
					$receiver[1]->email = $userProfile['UserProfile']['paypal_account'];
					$receiver[1]->amount = round($receiver_amount);
					$receiver[1]->primary = false;
					$receiverList = new ReceiverList($receiver);
                    $currencty_code = 'USD';	
                    $slug = $projectFund['Project']['slug'];
					$payRequest = new PayRequest(new RequestEnvelope("en_US"), 'PAY_PRIMARY','https://buskerdues.com/payments/cancel_payment/' . $projectFund['ProjectFund']['id'] . '/' . ConstProjectFundStatus::Backed.'/'.$slug, $currencty_code, $receiverList, 'http://buskerdues.com/payments/process_ipn/' . $projectFund['ProjectFund']['id'] . '/' . ConstProjectFundStatus::Backed.'/'.$slug, 'http://buskerdues.com/payments/success_payment/' . $projectFund['ProjectFund']['id']. '/' . ConstProjectFundStatus::Backed.'/'.$slug);
		    $this->log($payRequest,'payRequest');

		    //$this->log($projectFund['ProjectFund']);

					$service = new AdaptivePaymentsService(Configuration::getAcctAndConfig($paypal_settings['PaymentGateway']['adaptive_API_UserName'], $paypal_settings['PaymentGateway']['adaptive_API_Password'], $paypal_settings['PaymentGateway']['adaptive_API_Signature'], $paypal_settings['PaymentGateway']['adaptive_API_AppID'], $paypal_settings['PaymentGateway']['is_test_mode']));
					try { 
						$response = $service->Pay($payRequest);
						$this->log($response,'paymentgatewayss');
						if(!empty($response))
						{ 
							$ack = strtoupper($response->responseEnvelope->ack);
							if(!empty($response->payKey) && $ack == "SUCCESS")
							{ 
                                //$this->ProjectFund->updateStatus($projectFund['ProjectFund']['id'], ConstProjectFundStatus::Backed, ConstPaymentGateways::Paypal);						
								$this->ProjectFund->updateAll(array('ProjectFund.paypal_pay_key' => "'".$response->payKey."'"), array('ProjectFund.id' => $projectFund['ProjectFund']['id']));
								$payPalURL = PAYPAL_REDIRECT_URL . '_ap-payment&paykey=' .$response->payKey;
								$this->log('Pay-key'); $this->log($response->payKey);
								$this->redirect($payPalURL);
							}
							else
							{ $this->log('response'); $this->log($response);
							  $error_res = json_encode((array)$response->error);
							  $error_resp = json_decode($error_res);
						      $this->log($error_resp,'MAGIC');
							  $this->Session->setFlash(__l('Your payment could not be processed due to an error' . print_r($response, true)) , 'default', null, 'error');
							  $this->redirect(array(
									'controller' => 'project_funds',
									'action' => 'add',
					...
					} catch(Exception $ex) { $this->log('ex'); $this->log($ex);
						$this->Session->setFlash(__l('Your payment could not be processed due to an error') , 'default', null, 'error');
						$this->redirect(array(
							'controller' => 'project_funds',
							'action' => 'add',
							$projectFund['ProjectFund']['id'],
