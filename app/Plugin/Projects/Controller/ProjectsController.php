...
class ProjectsController extends AppController
{
        ...
        if ((Configure::read('site.launch_mode') == 'Pre-launch' && $this->Auth->user('role_id') != ConstUserTypes::Admin) || (Configure::read('site.launch_mode') == 'Private Beta' && !$this->Auth->user('id'))) {
            ...
            if (!empty($this->request->params['named']['filter']) || !empty($this->request->params['named']['category'])) {
                ...
                switch ($this->request->params['named']['filter'] ?? '') {
                    ...
                    case 'featured':
                        $this->pageTitle.= ' - ' . __l('Popular Buskers');
                        $discover_heading.= __l('Popular Buskers');
                        $conditions['Project.is_featured'] = 1;
                        $response = Cms::dispatchEvent('Controller.ProjectType.getConditions', $this, array(
                            'data' => $projectType,
                            'type' => 'open'
                    ...
                    case 'ending_soon':
                        $this->pageTitle.= ' - ' . __l('Busks Ending Soon');
                        $discover_heading.= __l('Busks Ending Soon');
                        // $conditions['Project.project_end_date >= '] = date('Y-m-d');
                        ...
                        break;
                        case 'newest_artists':
                        $this->pageTitle.= ' - ' . __l('Newest Buskers');
                        $discover_heading.= __l('Newest Buskers');
                        $conditions['Project.created > '] = date('Y-m-d', strtotime("-30 days"));
                        $response = Cms::dispatchEvent('Controller.ProjectType.getConditions', $this, array(
                            'data' => $projectType,
                            'type' => 'open'
                        ));
                    ...
                    case 'successful':
                        $this->pageTitle.= ' - ' . __l('Ended Busks');
                        $discover_heading.= __l('Ended Busks');
                        $response = Cms::dispatchEvent('Controller.ProjectType.getConditions', $this, array(
                            'data' => $projectType,
                            'type' => 'closed'
                        ));
            ...
            if (!empty($this->request->params['named']['category'])) {
                ...
                if ($projectTypeName != null and  class_exists($projectTypeName)) {
                    ...
                    $response = $model->getViewConditions();
                    if(!empty($response)) {
                        $conditions['NOT'] = array_merge($conditions['NOT'], $response['NOT']);
                    }
				}
            }
            ...
            if (isset($projectTypeName) and  class_exists($projectTypeName)) {
                $model11 = new $projectTypeName();
                $dbo111  = $model11->getDatasource();
                $logData = $dbo111->getLog();
    ...
    public function add($project_id = '', $form_field_step = '')
    {
        ...
        $this->log($this->request->params,'demo_log');
        $this->log($this->request->data,'demo_log');
        if (!empty($this->request->params['named']['project_type'])) {
            if (!isPluginEnabled(ucfirst($this->request->params['named']['project_type']))) {
                $this->redirect(array(
                    'controller' => 'projects',
        ...
        if (!empty($form_field_step)) {
            ...
            if (empty($this->request->data['Project']['project_end_date'])) {
                ...
                // $end_date = explode('-', date('Y-m-d', strtotime('+365 days')));
                ...
                // $this->request->data['Project']['project_end_date']['month'] = '00';
                // $this->request->data['Project']['project_end_date']['day'] = '00';
                // $this->request->data['Project']['project_end_date']['year'] = '0000';
            }
        }
        if (isPluginEnabled('Idea')) {
            ...
            if (!empty($this->request->data['Project']['step']) && $this->request->data['Project']['step'] == 2) {
                ...
                if ($this->Project->validates() &$this->Form->validates() &$this->Project->Attachment->validates() &empty($response->data['error'][$projectType['ProjectType']['name']]) &$is_addrees_ok &empty($response->data['error']['ProjectReward']) &empty($is_payout_error)) {
                    ...
                    if ($this->Project->save($this->request->data, false)) {
                        ...
                        if(!empty($userProfile['UserProfile']['paypal_account'])&&!empty($userProfile['UserProfile']['paypal_first_name'])&& !empty($userProfile['UserProfile']['paypal_last_name']))     
                        {
                            $data['Project']['is_draft'] = 0;
                        }
                        $data['Project']['id'] = $projectId;
                        ///////
                        $data['Project']['fund_cycle'] = $this->request->data['Pledge']['fund_cycle_id'];
                        ////////
        ...
        ////////////
        $fundCycles[ConstFundCycles::seven] = '7';
        $fundCycles[ConstFundCycles::fourteen] = '14';
        $fundCycles[ConstFundCycles::thirty] = '30';
        $this->set(compact('fundCycles'));
        ////////////
        ...
        $chkProfile = $this->Project->User->UserProfile->find('first', array(
            'conditions' => array(
        ...
        if (empty($this->request->data['Project']['step']) && empty($this->request->params['named']['step'])) {
            if(!empty($chkProfile['UserProfile']['paypal_account'])&&!empty($chkProfile['UserProfile']['paypal_first_name'])&& !empty($chkProfile['UserProfile']['paypal_last_name']) || $_SESSION['Auth']['User']['role_id'] == 1)     
            {
              $this->render('guidelines');
            }else{
                $this->Session->setFlash(__l('Project owner must provide the Paypal Information to proceed further'), 'default', null, 'error');
                $this->redirect(array(
    ...
    public function edit($id = null, $form_field_step = null)
    {
        ...
        if (!empty($this->request->data) && $this->request->isPost()) {
            ...
            if ($this->Project->validates() &$this->Form->validates() &$this->Project->Attachment->validates() &$this->Project->City->validates() &$this->Project->State->validates() &empty($response->data['error'][$project['ProjectType']['name']]) &empty($response->data['error']['ProjectReward']) ) {
                ...
                ///////
                $this->request->data['Project']['fund_cycle'] = $this->request->data['Pledge']['fund_cycle_id'];
                ////////
                if ($this->Project->save($this->request->data)) {
                    ...
                    $is_overwrite_data = 1;
                    $this->Session->setFlash(sprintf(__l('%s could not be updated. Please, try againx.') , Configure::read('project.alt_name_for_project_singular_caps')) , 'default', null, 'error');
                }
            } else {
                $is_overwrite_data = 1;
                $this->Session->setFlash(sprintf(__l('%s could not be updated. Please, try againxx.') , Configure::read('project.alt_name_for_project_singular_caps')) , 'default', null, 'error');
                if (!empty($is_payout_error)) {
                    $this->Session->setFlash(__l('You must connect atleast one gateway') , 'default', null, 'error');
                }
            }
	 ...
	 public function renview($id = null, $form_field_step = null)
    {
        $_SESSION['post_action'] = 'edit';
        $this->pageTitle = sprintf(__l('Edit %s') , Configure::read('project.alt_name_for_project_singular_caps'));
        $this->loadModel('Projects.Form');
    ...
    function createNotification($title = '', $slug = '')
    {
        $data = ["title" => $title, "url" => 'https://buskerdues.com/project/'.$slug];
        ...
        $ref = $firebase->getReference('new_project')->getSnapshot();
        ...
        if (!empty($this->request->data['Project'])) {
            ...
            if ($actionid && !empty($projectIds)) {
                if ($actionid == ConstMoreAction::Inactive) {
                    ...
                    $this->createNotification($project['Project']['name'], $project['Project']['slug']);
