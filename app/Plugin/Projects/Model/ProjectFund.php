...
class ProjectFund extends AppModel
{
    ...
    function updateFundCount($projectFund)
    {
        ...
        if (!empty($projectFund['ProjectFund']['user_id'])) {
			...
			if (!empty($_SESSION['Auth']['User']['Role']['id'])) { 
    ...
    public function updateStatus($project_fund_id, $project_fund_status_id, $payment_gateway_id = null, $is_canceled_from_gateway = 0)
    {
        ...
        $this->
        {
            ...
            // if ($project_fund_status_id == 1) {
            //     $this->addDaysToEndDate($project['Project']['id']);
            // }
        }
        // Backed //
            ...
            if (in_array($projectFund['ProjectFund']['project_fund_status_id'], array(
                ...
                if ($project['Project']['project_type_id'] == ConstProjectTypes::Donate) {
					...
					if ($projectFund['ProjectFund']['payment_gateway_id'] == ConstPaymentGateways::SudoPay) {
						...
						if(!empty($connected_gateways) && in_array($projectFund['ProjectFund']['sudopay_gateway_id'], $connected_gateways)) {
							...
							$response = $s->callMarketplaceCapture($post);
							if (!empty($response->error->code)) {
								$return['error'] = 1;
								$return['error_message'] = $response->error->message;
                            }
                        }else if ($projectFund['ProjectFund']['payment_gateway_id'] == ConstPaymentGateways::Paypal) { 
             ...
             //   $this->log('project type logged');
	       //                $this->log($project['ProjectType']['name']);

            $this->Project->updateProjectOnFund($projectFund, $project);


            //		$this->log("project type data");
            //		$this->log($project);
                    
                    $projectTypeName = ucwords($project['ProjectType']['name']);
                    App::import('Model', $projectTypeName . '.' . $projectTypeName);
                    $model = new $projectTypeName();
            //		$this->log($prjoectTypeName);
            //		$this->log($model);
            ...
            $model->updateProjectStatus($projectFund['ProjectFund']['id']);
        ...
        function processStatus2($projectFund, $payment_gateway_id, $project)
        {
            ...
            if ($projectFund['ProjectFund']['payment_gateway_id'] == ConstPaymentGateways::SudoPay) {
            ...
            } elseif ($projectFund['ProjectFund']['payment_gateway_id'] == ConstPaymentGateways::Paypal) {    
                $return['error'] = 1;
				if(!empty($projectFund['ProjectFund']['paypal_pay_key']))
				{
					App::import('Model', 'PaymentGateway');
					$this->PaymentGateway = new PaymentGateway();
					$paypal_settings = $this->PaymentGateway->getPaymentSettings(ConstPaymentGateways::Paypal);
					// to call the execute payment method to pay the amount to the host
					...
					$refundRequest->payKey = $projectFund['ProjectFund']['paypal_pay_key'];
					...
					try {
						...
						{ 
							...
							if(empty($response->error) && $ack == "SUCCESS")
							{
								$return['error'] = 0;
			 ...
			 // Paypal adaptive refund process end
        ...
        function processStatus3($projectFund, $payment_gateway_id, $project)
        {
            ...
            if ($projectFund['ProjectFund']['payment_gateway_id'] == ConstPaymentGateways::SudoPay) {
                ...
                $this->log('CAPTURE-1','capture_log');
                $this->log($projectFund,'capture_log');
                if(!empty($projectFund['ProjectFund']['id']))
				{
					if(!empty($projectFund['ProjectFund']['paypal_pay_key']))
					{
						$this->log('=====second pay starts=====');
						App::import('Model', 'PaymentGateway');
						$this->log('=====step111111111=====');
						$this->PaymentGateway = new PaymentGateway();
						$this->log('=====step2222222=====');
						$paypal_settings = $this->PaymentGateway->getPaymentSettings(ConstPaymentGateways::Paypal);
						$this->log('=====step3333333=====');
						// to call the execute payment method to pay the amount to the host
						App::import('Vendor', 'paypal-adaptive/PPAutoloader');
						$this->log('=====step444444=====');
						$this->PPAutoloader = new PPAutoloader();
						...
						$this->PPAutoloader->register($paypal_settings['PaymentGateway']['is_test_mode']);
						$this->log('=====step66666=====');
						$executePaymentRequest = new ExecutePaymentRequest(new RequestEnvelope("en_US"),$projectFund['ProjectFund']['paypal_pay_key']);
						$this->log('=====step77777=====');
						$executePaymentRequest->actionType = 'PAY';
						$this->log('=====step8888888=====');
						//$service = new AdaptivePaymentsService(Configuration::getAcctAndConfig());
						$service = new AdaptivePaymentsService(Configuration::getAcctAndConfig($paypal_settings['PaymentGateway']['adaptive_API_UserName'], $paypal_settings['PaymentGateway']['adaptive_API_Password'], $paypal_settings['PaymentGateway']['adaptive_API_Signature'], $paypal_settings['PaymentGateway']['adaptive_API_AppID'],$paypal_settings['PaymentGateway']['is_test_mode']));
						try {
							$response = $service->ExecutePayment($executePaymentRequest);
							$this->log('second pay response');
							$this->log($response);
							if(!empty($response))
							{ 
								$ack = strtoupper($response->responseEnvelope->ack);
								$this->log('second pay $ack');
								$this->log($ack);
								if(empty($response->error) && $ack == "SUCCESS")
								{
									$this->log('second pay SUCCESS');									
									...
									$this->log('second pay SUCCESS updated');	
									//Host amount cleared log for moving amount to host's paypal account
									$this->log('second pay SUCCESS transacation updated');	
								}
							}
						} catch(Exception $ex) {
							$this->log('second pay exception error');
							$this->log($ex);
						}
						$this->log('=====second pay ends=====');
					}
				}   
        ...
        function addDaysToEndDate($project_id = null)
        {
            if (!empty($project_id)) {
