...
if(!empty($this->request->params['named']['project_fund_id'])){
  ...
  <div class ="pull-right">
    <div class ="pull-left textb"><?php echo __l('Sort:'); ?></div>
    <ul class ="unstyled pull-right">
    <?php $stat_class = (!empty($this->request->params['named']['filter']) && $this->request->params['named']['filter'] == 'freshness') ? 'blackc' : null; ?>
    <li class = "span pull-left"><?php echo $this->Html->link(__l('Freshness'), array('controller' => 'messages', 'action' => 'index', 'project_id' => $project['Project']['id'],'project_fund_id'=>$project_fund_id, 'filter' => 'freshness', 'admin' => false, 'plugin' => 'projects'), array('class' => 'js-message-link js-no-pjax ' . $stat_class, 'title' => __l('Freshness')));?></li>
    <?php $stat_class = (!empty($this->request->params['named']['filter']) && $this->request->params['named']['filter'] == 'descending') ? 'blackc' : null; ?>
    <li class = "span pull-left"><?php echo $this->Html->link(__l('Descending'), array('controller' => 'messages', 'action' => 'index', 'project_id' => $project['Project']['id'],'project_fund_id'=>$project_fund_id, 'filter' => 'descending', 'admin' => false, 'plugin' => 'projects'), array('class' => 'js-message-link js-no-pjax ' . $stat_class, 'title' => __l('Descending')));?></li>
    <?php $stat_class = ((empty($this->request->params['named']['filter'])) || (!empty($this->request->params['named']['filter']) && $this->request->params['named']['filter'] == 'ascending')) ? 'blackc' : null; ?>
    <li class = "span pull-left"><?php echo $this->Html->link(__l('Ascending'), array('controller' => 'messages', 'action' => 'index', 'project_id' => $project['Project']['id'],'project_fund_id'=>$project_fund_id, 'filter' => 'ascending', 'admin' => false, 'plugin' => 'projects'), array('class' => 'js-message-link js-no-pjax ' . $stat_class, 'title' => __l('Ascending')));?></li>
    </ul>
  </div>
  </div>
  <?php } ?>
  <div class="nav nav-pills well space nav-tabs no-round no-bor clearfix">
  <?php if (!empty($messages)) { ?>
...
<div class="sep-bot">
  ...
  <li class="<?php echo $path_class;?>">
    ...
    <div class="<?php echo $reply_path; ?>">
      <?php if($path_count != 0) {?>
      <div class="sep-top">
      <?php } ?>
      <div class = "clearfix top-space">
        <div class="span"><?php echo $this->Html->getUserAvatar($message['OtherUser'], 'micro_thumb'); ?></div>
        <div class="<?php echo $span_class; ?> pull-left">
        <p class="no-mar clearfix"><?php echo $this->Html->link($this->Html->cText($message['OtherUser']['username']), array('controller'=> 'users', 'action' => 'view', $message['OtherUser']['username'], 'admin' => false), array('escape' => false, 'title'=>$this->Html->cText($message['OtherUser']['username'],false)));?> <span> <?php echo '-'; ?> </span>
        <?php
        ...
        ?>
        <?php echo $this->Html->link(__l('Reply'),array('controller'=>'messages','action'=>'compose',$message['Message']['id'],'reply','user'=>$message['OtherUser']['username'], 'project_id' => $message['Project']['id'], 'reply_type' => 'quickreply', 'root' => $message['Message']['root'], 'm_path' => $message['Message']['materialized_path']), array("class" =>"btn btn-primary btn-small pull-right reply-".$message['Message']['id']." js-link-reply js-no-pjax {'container':'js-quickreply-" . $message['Message']['id'] . "','responsecontainer':'js-quickreplydiv-" . $message['Message']['id']."'}", 'title' => __l('Reply'))); ?></p>
        <?php }?>
        <?php }?>
        <div class="js-quickreplydiv-<?php echo $message['Message']['id'];?> clearfix"></div>
      </div>
      <?php if($path_count != 0) {?>
      </div>
      <?php } ?>
