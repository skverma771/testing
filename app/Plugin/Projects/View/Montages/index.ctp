  ...
  <div class="project-block" id="js-pledge-trending_performance-scroll">
    <div class="montage-main">
	  ...
	  //print_r($project_categories); ?>
			...
			<option value = "<?php echo Router::url('/', true);?>montages">All Category</option>
			 <?php 
				...
				<?php	$no_mar_class='';
					...
					<?php						
												if (!empty($montage['Montage']['attachment_id']) && empty($montage['Montage']['video_url'])) {
                                                    ...
                                                    if(strpos($montage['Montage']['video_url'], 'youtu.be') !== false) {
                                                        $pattern = '%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i';
                                                        if (preg_match($pattern, $montage['Montage']['video_url'], $match)) {
                                                            $ytID = $match[1];
                                                        }
                                                    } else {
                                                    ...
                                                    ?>
														<img src="https://img.youtube.com/vi/<?php echo $ytID; ?>/hqdefault.jpg"></img>
												<?php
												} else {?>
