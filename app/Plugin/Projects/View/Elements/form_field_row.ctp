<?php
  ...
  if (!empty($field['is_deletable'])) {
    $row_delete = $this->Html->link('<i class="icon-remove"></i> ' . __l('Remove'), array('controller' => 'form_fields', 'action' => 'delete', $field['id']), array('class' => 'js-no-pjax js-form-field-delete', 'title' => __l('Remove'), 'escape' => false));
  } else {
  ...
  $row = array(
    ...
    $row_active,
    $row_delete . $this->Html->link(' <i class="icon-edit"></i> ' . __l('Depends On'), array('controller' => 'form_fields', 'action' => 'edit', $field['id']), array('data-toggle' => 'modal', 'data-target' => '#js-edit-ajax-modal', 'class' => 'js-no-pjax' ,'title' => __l('Depends On'), 'escape' => false))
  );
