<div class="ver-space">
  <div class="sep-top page-header no-bor tooltip-left-align">
   <span class="span project-logo js-tooltip show-inline" title="pushing the button allows viewer to tip a performance; the offered amount will captured and held in a Busker's account until the project end date is reached, funds are then released." data-placement="bottom">
   <?php echo $this->Html->image($project_type.'-s-icon.png', array('alt'=>$project_type, 'width' => '67', 'height' => '67')); ?> </span>
   <h2 class="dc pr ver-space"><span class="or-hor pa linkc lightgrey-bg mob-ps"><?php echo __l('Start Project');?></span></h2>
  ...
  <div class="clearfix page-header no-bor top-space">
    ...
    <div><?php echo $this->Html->link(__l('Work flow'), array('controller' => 'how-it-works', 'admin' => false), array('title' => __l('Work flow'), 'class' => 'js-tooltip btn btn-module')); ?></div>
    </div>
  </div>
  </div>
  <?php if (isPluginEnabled('Idea')): ?>
