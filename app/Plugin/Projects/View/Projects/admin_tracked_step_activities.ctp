...
?>
	<div class="textb"><?php echo $formFieldStep['FormFieldStep']['name']; ?></div>
	<?php if(!empty($val['submitted_on'])): ?>
		...
		<?php if (!empty($val['approved_on'])): ?>
			<div><?php echo __l('Approved On: ') . $this->Html->cDateTimeHighlight($val['approved_on']); ?></div>
			<div><?php echo __l('Private Note: ') . $val['private_note'][$i]; ?></div>
		<?php endif; ?>
	<?php endif; ?>
<?php
			endif;
