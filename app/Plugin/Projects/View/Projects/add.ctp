									...
									<?php
										foreach($FormFieldGroup['FormField'] as $key => $FormField) {
											...
											if (count($pledgeTypes) > 1) {
												...
												// $fund_cycle_val = array_keys($fundCycles, Configure::read('Project.is_default_fund_cycle'));
				...
				<div class="pull-left draft "><?php echo $this->Form->submit(__l('Draft'), array('class' => 'btn btn-module blackc', 'name' => 'data[Project][draft]', 'div' => false));?></div>
					<div class="span14">
						<?php if ($this->request->data['Project']['form_field_step'] != 1): ?>
							<div class="pull-left hor-mspace "><?php echo $this->Html->link(__l('Back'), array('controller' => 'projects', 'action' => 'add', 'project_type' => $project_type_slug, $this->request->data['Project']['id'], $this->request->data['Project']['form_field_step']-1), array('class' => 'btn btn-module')); ?></div>
						<?php endif; ?>
