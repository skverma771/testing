...
if( ($this->request->params['named']['project_type'] ?? '') == 'pledge' && ($this->request->params['named']['filter'] ?? '') != 'browse' && empty($this->request->params['named']['view'])) {
	...
	if($select_category == 'recommended') { 
		...
		$select_h = 'selected';
	} else if($select_category == 'newest_artists') {
		$select_j = 'selected';
	}
		?>
...
<div class="bot-mspace clearfix pledge-block creative-ideas tip-block" itemtype="http://schema.org/Product" itemscope>
	<div class="space row">
      ...
      <option value = 'newest_artists' <?php echo $select_j; ?>>Newest Buskers</option>
			<option value = 'recommended' <?php echo $select_r; ?>>Recommended Buskers</option>
			<option value = 'popular' <?php echo $select_p; ?>>Popular Buskers</option>
			<option value = 'almost_funded' <?php echo $select_a; ?>>Almost Funded</option>
			<option value = 'ending_soon' <?php echo $select_e; ?>>Busks Ending Soon</option>
			<option value = 'hall_of_fame' <?php echo $select_h; ?>>Ended Busks</option>
		</select>
		</div>
		<div class="span5">
...
<?php
  ...
  foreach ($projects as $project):
    ...
    if($project['Pledge']['pledge_project_status_id'] == ConstPledgeProjectStatus::FundingExpired):
        // continue;
    endif;
