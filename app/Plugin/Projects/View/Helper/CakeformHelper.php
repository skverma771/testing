                    ...
                    if (!empty($field['is_reward'])) {
                      //  if ($j == 1) {
							...
							if ($j == 1) {
								...
								$out.= '<div class="clearfix add-block pull-right cur"><span class ="js-add-more js-no-pjax btn clone-add "><i class="icon-plus"></i> ' . __l("Add More") . '</span></div>';
							}
                            if (empty($this->request->data['ProjectReward'])) {
                                if (!empty($field['reward'])) {
									$out.= '<div class="website-block js-field-list reward-block clearfix top-space">'; 
									$out.= '<div class=" ' . $class . '">';
                                ...
                                if($j == 1) {
									...
									foreach($this->request->data['ProjectReward'] as $key => $projectReward) {
										$reward = '';
										if ($i != 0) {
											$reward = ' reward-clone sep-top top-space ';
										}
										$out.= '<div class="reward-block clearfix js-field-list js-new-clone-' . $i . $reward . '">';
										if ($key) {
											$out.= '<span class="js-website-remove js-no-pjax btn clone-remove pull-right cur"><i class="icon-remove space"></i>' . __l('Remove') . '</span>';
										}
										...
										if (!empty($projectReward['is_shipping'])) {
										...
										if (!empty($projectReward['estimated_delivery_date'])) {
										...
										if (!empty($projectReward['is_having_additional_info'])) {
										...
										if (!empty($projectReward['additional_info_label'])) {
            ...
            if (!empty($field['name'])) {
                ...
                if ($field['name'] == 'Attachment.filename') {
                    if (!empty($this->request->data['Attachment']) && !empty($this->request->data['Project']['name'])) {
                        ...
                        $options['size'] = 33;
                        $out.= '<div class="upload-image offset5 space">';
                ...
                if ($field['name'] == 'Project.description') {
                    $options['class'] = 'js-editor span16 no-mar descblock {"is_html":"false"}';
                    $options['rows'] = false;
                    $options['cols'] = false;
                }
