...
class EmbedHelper extends AppHelper
{
        ...
        foreach($embeds as $embed) {
            if (preg_match('~' . $embed['url-match'] . '~imu', $url, $match)) {
                ...
                if (isset($embed['fetch-match'])) {
					...
					if($this->request->params['controller'] == 'home_videos'){
						$this->_setHomePageVideoParams();
					}else if($this->request->params['controller'] == 'success_stories'){
    ...
    public function hotFixEmbed($url, $text = null)
    {
        if(strpos($url, 'shorts') !== false) {
            $arr = explode('/', $url);
            
            return '<div class="video-container"><span class="overlay-text">'.$text.'</span><iframe width="100%" height="324" src="https://www.youtube.com/embed/'.end($arr).'" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
        }

        if(strpos($url, 'youtu') !== false) {
            $pattern = '%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i';
            if (preg_match($pattern, $url, $match)) {
                return '<div class="video-container"><span class="overlay-text" style="display: none">'.$text.'</span><iframe width="100%" height="324" src="https://www.youtube.com/embed/'.$match[1].'" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
            }

            return false;
        } else if(strpos($url, 'vimeo') !== false) {
            $pattern = '%(?:www\.)?vimeo\.com/([0-9]{1,12})%i';
            if (preg_match($pattern, $url, $match)) {
                return '<div class="video-container"><iframe src="https://player.vimeo.com/video/'.$match[1].'?h=bc09f279fd" width="100%" height="360" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe></div>';
            }
        } else if(strpos($url, 'rumble') !== false) {
            $html = file_get_contents( $url );
            $pattern = preg_match('/"video\":\"(.*?)"/', $html, $match);
            if (current($parsed_arr)) {
                return '<div class="video-container"><iframe class="rumble" width="640" height="360" src="https://rumble.com/embed/'.current($parsed_arr).'/?pub=4" frameborder="0" allowfullscreen></iframe></div>';
            }
        }
    }
	...
	private function _setHomePageVideoParams() 
    {
        $source = $this->_embed['embed-src'];
        $flashvars = (isset($this->_embed['flashvars'])) ? $this->_embed['flashvars'] : null;
        for ($i = 1; $i <= count($this->_media_id); $i++) {
