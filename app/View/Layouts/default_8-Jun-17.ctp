...
<?php if(end($parts)!='how-it-works' &&	end($parts)!='success_stories'){ ?>
  ...
  if(($this->request->params['controller'] == 'projects' && $this->request->params['action'] == 'index') && !isset($this->request->params['named']['filter']) && (empty($this->request->params['named']['category']) && $this->request->params['named']['project_type'] != 'home')) {
  	$main_cont = '';
	}
