<div class="hor-space bot-mspace payment-gateways payment-type ver-space">
	<?php 
		...
		foreach($gateway_types As $key => $gateway_type) {
			...
			if(!empty($gateway_type['display_name']) && $gateway_type['id'] == '4'){
				$gateway_type['display_name'] = 'Pay with PayPal';
			}
	...
	<div id="paymentgateways-tab-container" class="">
		<ul class="nav nav-tabs no-mar">
			...
			<li><a href="#paymentGateway-<?php echo $gateway_group['id']; ?>" class="js-no-pjax" data-toggle="tab"><div>
				...
				foreach($payment_gateways AS $payment_gateway) {
					...
					if($payment_gateway['group_id'] == $gateway_group['id']) {
						if ($payment_gateway['payment_id'] == ConstPaymentGateways::Wallet) {
							$option_value = '<div class="pull-left">' . $this->Html->image('wallet-icon.png', array( 'alt' => __l('Wallet'))) . '<span class="show">' . $payment_gateway['display_name'] . '</span></div>';							
						} else {
						...
						if ($payment_gateway['group_id'] == 4922):
							...
							echo $this->Form->input($model.'.payment_gateway_id', array('id' => 'PaymentGatewayId', 'legend' => false, 'type' => 'radio', 'label'=> true, 'div' => 'span4', 'options' => $options, 'data-sudopay_form_fields_tpl' => $template, 'class' => 'js-payment-type js-no-pjax pull-left', 'checked' => $checked));
						endif;
					}
				}
				?>	
...
<div class="submit-block form-payment-panel clearfix">
	<div class="submit">
		...
		<div class= "js-wallet-connection hide">
			...
			<?php
				echo $this->Form->submit(__l('Pay with Wallet'), array('name' => 'data['.$model.'][wallet]', 'class' => '{"balance":"' . $logged_in_user['User']['available_wallet_amount'] . '"}  btn-module wallet-button ' . ' js-update-order-field js-no-pjax', 'div' => false));
			?>
		</div>
		<div class= "js-paypal-adaptive">
			<?php
				echo $this->Form->submit(__l('Pay Now'), array('name' => 'data['.$model.'][paypal]', 'div' => false, 'class' => 'btn text-13'));
			?>
		</div>		
		<?php /*?><div class= "js-normal-sudopay hide">
		<?php
				echo $this->Form->submit(__l('Pay Now'), array('name' => 'data['.$model.'][sudo]', 'div' => false, 'id' => 'sudopay_button'));
			?>
		</div><?php  */?>
	</div>   
</div>
