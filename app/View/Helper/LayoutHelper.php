...
class LayoutHelper extends AppHelper
{
    ...
    function adminMenus($menus, $options = array()) 
    {
        ...
        $out = '<div class="navbar no-mar"><div class="navbar-inner hor-space no-round clearfix"><a href="#" data-target=".nav-collapse" data-toggle="collapse" class="btn btn-navbar ver-mspace"> <i class="icon-align-justify icon-24 blackc"></i></a><div class="nav-collapse">';
        $out.= '<ul class="nav admin-menu row container-fluid"><li class="hor-space"><span class="top-mspace show top-space">' . $this->Html->image('logo.png', array(
            'width' => 100,
            'height' => 35
        )) . '</span></li>';
        $icon_class = '';
        ...
        foreach($sorted as $menu) {
            ...
            if ($menu['icon-class'] == 'admin-payment' && ($this->request->params['controller'] == 'pledges' || $this->request->params['controller'] == 'donates' || $this->request->params['controller'] == 'lends' || $this->request->params['controller'] == 'equities') && $this->request->params['action'] == 'admin_funds') {
                ...
                $out.= '<li class="pull-left dc sep dropdown pr ' . $active_class . $class . ' ">';
            }
            ...
            if ($menu['icon-class'] == 'admin-payment') {
                $out.= '<span class="text-20 over-hide icon-payment">$</span>';
            } else {
                $out.= '<span class="show"><i class="text-20 over-hide icon-' . $menu['icon-class'] . '"></i></span>';
            }
            $out.= '<span class="show textb">';
            $out.= $menu['title'];
            $out.= '</span></a>';
            if ($menu['icon-class'] == 'random') {
                ...
                $out.= "</li>";
				 $out.= "<li>";
				 $out.= $this->Html->link(__l('How it works') , array(
                ...
                if (!empty($menu['children'])) {
                    if (!empty($menu['column']) && $menu['column'] > 1) {
                        ...
                        if ($menu['icon-class'] == 'cogs') {
                            $out.= '<div>';
                            $out.= $this->Html->link('<span class="pull-right"><i class="icon-cog"></i>' . __l('Settings Overview') . '</span>', array(
                                'controller' => 'settings',
                                'action' => 'index'
                            ) , array(
                                'class' => '',
                        ...
                        if (empty($menu['is_hide_title'])) {
                            $out.= '<h6><span class = "span">' . $menu['title'] . '</span></h6>';
                        }
        ...
        foreach($menu['children'] as $child) {
            if ((empty($from) && empty($to)) || ($i >= $from && $i <= $to)) {
                ...
                if (!empty($child['url']['controller']) && $child['url']['controller'] == 'payment_gateways') {
                    $icon.= '<i class="icon-cog text-16 pull-left"></i>';
                }
                if (!empty($child['url'])) {
