...
class AppHelper extends Helper
{
	...
	public function getdays($project_enddate){
		App::import('Model', 'Project.Project');
		$time_strap = strtotime($project_enddate) - strtotime(date('Y-m-d'));
        $days = floor($time_strap/(60*60*24));
        if ($days > 0) {
            $total_days['day'] = $days;
        } else {
            $total_days['day'] = 0;
			$hours_strap = strtotime($project_enddate . ' 23:59:59') - strtotime(date('Y-m-d H:i:s'));
			$hours = floor($hours_strap/(60*60));
			if($hours > 0) {
				 $total_days['hour'] = $hours;
			}else{
				 $total_days['hour'] = 0;
			}
        }
	...
	public function userCounts() 
    {
		App::import('Model', 'User');
        $this->User = new User();
		$online_users = $this->User->find('count', array('conditions'=>array('is_online'=>1)));
		return $online_users;
	}
	
	function totalViewCounts() 
    {
        App::import('Model', 'Projects.TotalView');
        $this->TotalView = new TotalView();
        $viewers = $this->TotalView->find('count', array('conditions'=>array('view'=>1)));
        return $viewers;
    }
	function lastDay() 
    {
	...
	function lastWeek() 
    {
        App::import('Model', 'Projects.TotalView');
        $this->TotalView = new TotalView();
        $last_week = $this->TotalView->find(
		  'count',
		  array(
			'conditions' => array(
			  'TotalView.date >=' => date('Y-m-d H:i:s', strtotime('-7 day'))
			)
		  )
		);
        return $last_week;
	...
	function lastMonth() 
    {
        App::import('Model', 'Projects.TotalView');
        $this->TotalView = new TotalView();
        $last_month = $this->TotalView->find(
		  'count',
		  array(
			'conditions' => array(
			  'TotalView.date >=' => date('Y-m-d H:i:s', strtotime('-30 day'))
			)
		  )
		);
        return $last_month;
	...
	function lastYear() 
    {
        App::import('Model', 'Projects.TotalView');
        $this->TotalView = new TotalView();
        $last_year = $this->TotalView->find(
		  'count',
		  array(
			'conditions' => array(
			  'TotalView.date >=' => date('Y-m-d H:i:s', strtotime('-365 day'))
			)
		  )
		);
        return $last_year;
