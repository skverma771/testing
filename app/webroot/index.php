...
ini_set('error_log', ROOT."/app/webroot/pe.log");
...
include(ROOT.DS.'core'.DS.'lib'.DS.'Cake'.DS.'deb.php');
...
function updateViews($main_table_name,$main_field_name,$view_table_name,$main_id)
{
	...
	if($database->default['datasource']=='Database/Mysql') {
		$db = mysqli_connect($database->default['host'], $database->default['login'], $database->default['password']) or die('Error connecting to mysql');
		mysqli_select_db($db, $database->default['database']);
		mysqli_set_charset($db, 'utf8');
		if ($main_result = mysqli_query($db, 'SELECT * FROM ' . $main_table_name . ' WHERE id = ' . $main_id)) {
			$main_row = mysqli_fetch_assoc($main_result);
		}
		$ip_result = mysqli_query($db, 'SELECT id FROM `ips` WHERE ip = "' . $_SERVER['REMOTE_ADDR'] . '"');
		if (mysqli_num_rows($ip_result)) {
			$ip_row = mysqli_fetch_assoc($ip_result);
			$ip_id = $ip_row['id'];
		} else {
			if (!empty($_COOKIE['_geo'])) {
				$_geo = explode('|', $_COOKIE['_geo']);
				$country_result = mysqli_query($db, 'SELECT id FROM `countries` WHERE iso_alpha2 = "' . $_geo[0] . '"');
				if (mysqli_num_rows($country_result)) {
					$country_row = mysqli_fetch_assoc($country_result);
					$country_id = $country_row['id'];
				}
				$state_result = mysqli_query($db, 'SELECT id FROM `states` WHERE name = "' . $_geo[1] . '"');
				if (mysqli_num_rows($state_result)) {
					$state_row = mysqli_fetch_assoc($state_result);
					$state_id = $state_row['id'];
				}
				$city_result = mysqli_query($db, 'SELECT id FROM `cities` WHERE name = "' . $_geo[2] . '"');
				if (mysqli_num_rows($city_result)) {
					$city_row = mysqli_fetch_assoc($city_result);
					$city_id = $city_row['id'];
				}
			}
			mysqli_query($db,'INSERT INTO `ips` (`created`, `modified`, `ip`, `host`, `city_id`, `state_id`, `country_id`, `latitude`, `longitude`) VALUES (now(), now(), "' . $_SERVER['REMOTE_ADDR'] . '", "' . gethostbyaddr($_SERVER['REMOTE_ADDR']) . '", ' . $city_id . ', ' . $state_id . ', ' . $country_id . ', "' . $_geo[3] . '", "' . $_geo[4] . '")');
...
mysqli_query($db, 'INSERT INTO `' . $view_table_name . '` (`created`, `modified`, `user_id`, `' . $main_field_name . '_id`, `ip_id`, `project_view_type_id`) VALUES (now(), now(), ' . $user_id . ', ' . $main_id . ', ' . $ip_id . ', '. $project_view_type_id . ')');
$view_result = mysqli_query($db,'SELECT COUNT(*) as count FROM ' . $view_table_name . ' WHERE project_view_type_id = '.$project_view_type_id.' AND ' . $main_field_name . '_id = ' . $main_id);
$view_row = mysqli_fetch_assoc($view_result);
mysqli_query($db,'UPDATE `' . $main_table_name . '` SET '.$view_count_column.' = "' . $view_row['count'] . '" WHERE id = ' . $main_id);
} else {
	mysqli_query($db, 'INSERT INTO `' . $view_table_name . '` (`created`, `modified`, `user_id`, `' . $main_field_name . '_id`, `ip_id`) VALUES (now(), now(), ' . $user_id . ', ' . $main_id . ', ' . $ip_id . ')');
	if($view_result = mysqli_query($db,'SELECT COUNT(*) as count FROM ' . $view_table_name . ' WHERE ' . $main_field_name . '_id = ' . $main_id)) {
		$view_row = mysqli_fetch_assoc($view_result);
		mysqli_query($db,'UPDATE `' . $main_table_name . '` SET ' . $main_field_name . '_view_count = "' . $view_row['count'] . '" WHERE id = ' . $main_id);
	}
}
} else {
...
function updateViews1($main_table_name,$main_field_name,$view_table_name,$main_id)
{
	require '..' . DS . 'Config' . DS . 'database.php';
	$database = new DATABASE_CONFIG();
	if($database->default['datasource']=='Database/Mysql') {
