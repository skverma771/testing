...
function fcn_addslashes(str) {
	...
	str = new String(str);
	var str = str.replace(/\\/g, '\\\\').replace(/\u0008/g, '\\b').replace(/\t/g, '\\t').replace(/\n/g, '\\n').replace(/\f/g, '\\f').replace(/\r/g, '\\r').replace(/'/g, '\\\'').replace(/"/g, '\\"');
	return str.valueOf();
}
...
function loadAdminPanel() {
	if ($.cookie('_gz') != null && (window.location.href.indexOf('/project/') != -1 || window.location.href.indexOf('/user/') != -1)) {
		...
		if (typeof($('.js-project-view').data('project-id')) != 'undefined' &&  $('.js-project-view').data('project-id') !='' && $('.js-project-view').data('project-id') != null) {
			...
			var url = 'projects/show_admin_control_panel/view_type:project/id:'+pid;
		}
		if (url !='') {
...
(function() {
		...
		$('.js-payment-type').each(function() {
			...
			if ($this.prop('checked') == true) {
				...
				if ($this.val() == 2) {
					...
					//$('.js-paypal-adaptive').slideDown('fast');
					//$('.js-paypal-adaptive').addClass('fast');
					$('.js-wallet-connection').attr('style', 'display: block');
					$('.js-paypal-adaptive').attr('style', 'display: none');
				} else if ($this.val() == 4) {
					console.log('4');
					//console.log('under load');
					//$('.js-form, .js-instruction').addClass('hide');
					$('.js-wallet-connection').slideDown('fast');
					$('.js-wallet-connection').addClass('hide');
					//$('.js-normal-sudopay').slideDown('fast');
				  //$('.js-paypal-adaptive').slideUp('fast');	
					$('.js-wallet-connection').attr('style', 'display: none');
					$('.js-paypal-adaptive').attr('style', 'display: block');
				} else if ($this.val() == 1) {
	...
	//				console.log('test');
					for (var i in validation) {
						...
						if (
												(
													(
														typeof (validation[i]['rule']) != 'undefined' && typeof (validation[i]['rule'][0]) != 'undefined' && validation[i]['rule'][0] == 'custom' && (typeof (validation[i]['allowEmpty']) == 'undefined' || validation[i]['allowEmpty'] == false)
													)
													|| (
														typeof (validation['rule']) != 'undefined' && validation['rule'] == 'custom' && (typeof (validation['allowEmpty']) == 'undefined' || validation['allowEmpty'] == false)
													)
												) 
											) {
							let regExString = validation[i]['rule'][1];
														regExString = regExString.slice(1, regExString.length - 1);
														let customRegex = new RegExp(regExString);

	//												console.log('custom', customRegex, regExString);
													if ( !customRegex.test($(this).val()) ) {
																					error_message = 1;
																					break;
																				}
												}
		...
		$('div#js-comment-activity-section' + so).each(function(e) {
			FB = null;
			$.getScript('//connect.facebook.net/en_US/all.js', function(data) {
		...
		$('.js-wallet' + so + ', .js-payment-all' + so).each(function(e) {
			...
			$('.js-payment-status').each(function() {
				...
				if (htmlContent.indexOf('icon-ok') != '-1') {
		...
		$('#UserAddressSearch' + so + ', #ProjectAddressSearch' + so).each(function() {
            ...
            var google_map_key = '//maps.google.com/maps/api/js?key=AIzaSyAhHH1mjVdpO9Wiv9DjnGIROyBvAN-A-Ps&sensor=false&callback=loadSideMap&language=' + __cfg('user_language');
		...
		$('a.js-given').click(function() {
			...
			$.get($this.attr('href'), function(data) {
				if (data == 1) {
					$this.html('<i class="icon-thumbs-up"></i> ' + __l('Not given'));
				} else {
					$this.html('<i class="icon-thumbs-down"></i> ' + __l('Given'));
				}
				$this.unblock();
			});
			return false;
	...
	$.p.fpledgetypekey = function(selector) {
		$(selector).each(function(e) {
			...
			_this.parents('.js-clone').append('<div class="js-field-list top-space ' + cloneClass + '"><span class="span2 js-website-remove btn clone-remove pull-right cur"><i class="icon-remove"></i>' + __l(' Remove') + '</span>' + field_list.html() + '</div>');
	...
	$dc.ready(function($) {
        ...
        $dc.on('click', 'a:not(.js-no-pjax, .close):not([href^=http], #adcopy-link-refresh, #adcopy-link-audio, #adcopy-link-image, #adcopy-link-info)', function(event) {
		...
		}).on('click', '.js-pagination a, .js-filter a, a.js-filter', function(e) {
			$this = $(this);
			...
			if ($this.val() == 2) {
				...
				$('.js-wallet-connection').attr('style', 'display: block');
				$('.js-paypal-adaptive').attr('style', 'display: none');
				//$('.js-paypal-adaptive').addClass('hide');
			} else if ($this.val() == 4) {
				//console.log('4 clicked');
				//$('.js-normal-sudopay').addClass('fast');
				$('.js-wallet-connection').slideDown('fast');
				$('.js-wallet-connection').addClass('hide');
				//$('.js-gatway_form_tpl').hide();
				//$('.js-instruction').addClass('hide');
		   ...
		   //		$('.js-instruction').addClass('hide');
			...
			if ($('.sort' + groupId).hasClass('hide')) {
				...
				$(this).children().prop('class', 'icon-minus pull-right');
			} else {
				$('.sort' + groupId).addClass('hide');
				$(this).children().prop('class', 'icon-plus pull-right');
			}
		...
		}).on('change', '.js-select_model', function(e) {
			//var value = $(this).val();
			console.log($(this).val());
			if ($(this).val() == '120X120' || $(this).val() == '250X250' || $(this).val() == '400X400') {
				console.log('A');
				var elw = $(this).find('option:selected'); 
				console.log('B');
				var ahref = elw.attr("data-valued");
				console.log('C');
				var element = document.createElement('a');
				console.log('D');
				element.setAttribute('href',ahref);
				console.log('E');
			...
			/*if (value == '120X120') {
				var element = document.createElement('a');
				element.setAttribute('href',$(this).attr('data-value'));
				//element.setAttribute('download', filename);
				element.style.display = 'none';
				document.body.appendChild(element);
				element.click();
				document.body.removeChild(element);
			} else if (value == '250X250') {
				$('#js-listing-fee-type').html(' %');
			} else if (value == '400x400') {
				$('#js-listing-fee-type').html(' %');
			}*/
		...
		}).on('change', '.js-remove-error', function(e) {
			$(this).parents().find('.error-message').remove();
	        $(this).parent().removeClass('error');
		...
		}).on('blur', '.js-remove-error', function(e) {
			$(this).parents().find('.error-message').remove();
			$(this).parent().removeClass('error');
			...
			if (display == 'none') {
				$this.addClass('active-row').next('tr').show();
			} else {
				$this.removeClass('active-row').next('tr').hide();
			}
			...
			$.get($this.prop('href'), function(data) {
				if (data == 1) {
					$this.html('<i class="icon-thumbs-down"></i> ' + __l('Not Given'));
				} else {
					$this.html('<i class="icon-thumbs-up"></i> ' + __l('Given'));
				}
				$this.unblock();
				return false;
			});
			...
			$.get(__cfg('path_relative') + 'project_followers/add/' + project_id, function(data) {
				...
				$('#js-follow-id').addClass('btn span3 js-add-remove-followers js-tooltip js-unfollow');
				$('#js-follow-id').prop( {
					alt: 'Following',
					title: 'Unfollow',
					href: split_data[1]
			...
			$(this).html('<i class="icon-remove"></i> ' + __l('Unfollow'));
		}).on('mouseleave', '.js-unfollow', function(e) {
			$(this).html('<i class="icon-ok"></i> ' + __l('Following'));
			...
			if (document.location.pathname == __cfg('path_relative')) {
				...
				$('#header .js-header-top').addClass('pr z-top');
			} else {
				$('#header .js-header-menu').removeClass('site-header site-menu z-top');
				$('#header .js-header-top').removeClass('pr z-top');
			}
			...
			return false;
		}).on('click','.home-video',function(e){
			$('#home-video-hide').hide();
		}).on('change', '#MontageProjectId', function(e) {
			var project_id = 0;
            if ($(this).val() != '') {
                project_id = $(this).val();
            }
            var url = __cfg('path_relative') + 'admin/montages/get_project_details/' + project_id;
            $.get(url, function(data) {
				//alert(data);
                $('.content_part').html(data).unblock();
                return false;
            });
		});		
	}).ajaxStop(function() {
