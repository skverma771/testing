 ...
 */
	public function text($width = 72) {
		...
		if (!empty($description)) {
			$out[] = UtString::wrap($description, $width);
			$out[] = '';
		}
		...
		if (!empty($subcommands)) {
			...
			foreach ($subcommands as $command) {
				$out[] = UtString::wrap($command->help($max), array(
					'width' => $width,
		...
		if (!empty($options)) {
			...
			foreach ($options as $option) {
				$out[] = UtString::wrap($option->help($max), array(
					'width' => $width,
					'indent' => str_repeat(' ', $max),
					'indentAt' => 1
				));
		...
		if (!empty($arguments)) {
			...
			foreach ($arguments as $argument) {
				$out[] = UtString::wrap($argument->help($max), array(
					'width' => $width,
					'indent' => str_repeat(' ', $max),
					'indentAt' => 1
				));
		...
		if (!empty($epilog)) {
			$out[] = UtString::wrap($epilog, $width);
			$out[] = '';
		}
		return implode("\n", $out);
	}
